...
1. Thinking-блок в UI: отдельная карточка как у Claude `thinking` у pi-agent, или встроенный collapse в TurnCard? (Дефолт: отдельная, существующий паттерн ClaudeAgent.)
2. Синк OMP-skills только read-only, или с кнопкой «экспорт в craft skills»? (Дефолт: read-only.)
3. Прокси из mcpPool: единый `mcp__<source>__*` неймспейс как у pi — сохраняем?
