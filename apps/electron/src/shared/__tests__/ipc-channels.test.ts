import { describe, it, expect } from 'bun:test'
import { RPC_CHANNELS, type BroadcastEventMap } from '../types'

function flattenValues(obj: Record<string, unknown>): string[] {
  return Object.values(obj).flatMap(v =>
    typeof v === 'string' ? [v]
      : typeof v === 'object' && v !== null ? flattenValues(v as Record<string, unknown>)
      : []
  )
}

// Auto-generated by scripts/ipc-inventory.ts — DO NOT edit by hand.
const EXPECTED_CHANNELS: string[] = [
  'LLM_Connection:changed',
  'LLM_Connection:delete',
  'LLM_Connection:get',
  'LLM_Connection:getApiKey',
  'LLM_Connection:list',
  'LLM_Connection:listWithStatus',
  'LLM_Connection:refreshModels',
  'LLM_Connection:save',
  'LLM_Connection:setDefault',
  'LLM_Connection:setWorkspaceDefault',
  'LLM_Connection:test',
  'appearance:getDefaultZoomLevel',
  'appearance:getRichToolDescriptions',
  'appearance:setDefaultZoomLevel',
  'appearance:setRichToolDescriptions',
  'auth:logout',
  'auth:showDeleteSessionConfirmation',
  'auth:showDeleteWorkspaceConfirmation',
  'auth:showLogoutConfirmation',
  'automations:changed',
  'automations:delete',
  'automations:duplicate',
  'automations:get',
  'automations:getGraph',
  'automations:getHistory',
  'automations:getLastExecuted',
  'automations:replay',
  'automations:saveGraph',
  'automations:setEnabled',
  'automations:test',
  'badge:draw',
  'badge:draw-windows',
  'badge:refresh',
  'badge:setIcon',
  'browser-empty-state:launch',
  'browser-pane:click',
  'browser-pane:click-at',
  'browser-pane:create',
  'browser-pane:create-embedded',
  'browser-pane:destroy',
  'browser-pane:evaluate',
  'browser-pane:fill',
  'browser-pane:focus',
  'browser-pane:go-back',
  'browser-pane:go-forward',
  'browser-pane:interacted',
  'browser-pane:key',
  'browser-pane:list',
  'browser-pane:navigate',
  'browser-pane:reload',
  'browser-pane:removed',
  'browser-pane:resize',
  'browser-pane:screenshot',
  'browser-pane:scroll',
  'browser-pane:select',
  'browser-pane:snapshot',
  'browser-pane:state-changed',
  'browser-pane:stop',
  'browser-pane:sync-bounds',
  'browser-pane:type',
  'bundledSkills:CHANGED',
  'bundledSkills:getDisabled',
  'bundledSkills:list',
  'bundledSkills:setDisabled',
  'caching:getEnable1MContext',
  'caching:getExtendedPromptCache',
  'caching:setEnable1MContext',
  'caching:setExtendedPromptCache',
  'chatgpt:cancelOAuth',
  'chatgpt:completeOAuth',
  'chatgpt:getAuthStatus',
  'chatgpt:logout',
  'chatgpt:startOAuth',
  'cloudRuns:aggregate',
  'cloudRuns:cancel',
  'cloudRuns:deleteSchedule',
  'cloudRuns:getConfig',
  'cloudRuns:getEvents',
  'cloudRuns:getStatus',
  'cloudRuns:import',
  'cloudRuns:list',
  'cloudRuns:listArtifacts',
  'cloudRuns:listSchedules',
  'cloudRuns:readArtifact',
  'cloudRuns:resume',
  'cloudRuns:revokeShare',
  'cloudRuns:saveSchedule',
  'cloudRuns:sessionTopic',
  'cloudRuns:setConfig',
  'cloudRuns:share',
  'cloudRuns:submit',
  'collection:changed',
  'collection:getDisplay',
  'collection:setDisplay',
  'contextDocs:CHANGED',
  'contextDocs:acceptTemplate',
  'contextDocs:delete',
  'contextDocs:keepMineTemplate',
  'contextDocs:list',
  'contextDocs:read',
  'contextDocs:readTemplate',
  'contextDocs:write',
  'copilot:cancelOAuth',
  'copilot:deviceCode',
  'copilot:getAuthStatus',
  'copilot:logout',
  'copilot:startOAuth',
  'credentials:healthCheck',
  'debug:log',
  'deeplink:navigate',
  'dialog:openFolder',
  'drafts:delete',
  'drafts:get',
  'drafts:getAll',
  'drafts:set',
  'extensionHost:call',
  'extensionHost:getUrlAllowlist',
  'extensionHost:listCommands',
  'extensionHost:load',
  'extensionHost:mintCapability',
  'extensionHost:proxyFetch',
  'extensionHost:restart',
  'extensionHost:revokeCapability',
  'extensionHost:setUrlAllowlist',
  'extensionHost:start',
  'extensionHost:status',
  'extensionHost:statusAll',
  'extensionHost:stop',
  'extensionSurface:createEmbedded',
  'extensionSurface:destroy',
  'extensionSurface:focus',
  'extensionSurface:list',
  'extensionSurface:removed',
  'extensionSurface:stateChanged',
  'extensionSurface:syncBounds',
  'extensions:changed',
  'extensions:getState',
  'extensions:listCatalog',
  'extensions:listInstalled',
  'extensions:setEnabled',
  'file:generateThumbnail',
  'file:openDialog',
  'file:read',
  'file:readAttachment',
  'file:readBinary',
  'file:readDataUrl',
  'file:readPreviewDataUrl',
  'file:readUserAttachment',
  'file:storeAttachment',
  'fs:listDirectory',
  'fs:search',
  'gamification:award',
  'gamification:changed',
  'gamification:get',
  'git:getBranch',
  'gitbash:browse',
  'gitbash:check',
  'gitbash:setPath',
  'identity:changed',
  'identity:connect',
  'identity:disconnect',
  'identity:getState',
  'identity:refreshStatus',
  'identity:updateProfile',
  'input:getAutoCapitalisation',
  'input:getSendMessageKey',
  'input:getSpellCheck',
  'input:setAutoCapitalisation',
  'input:setSendMessageKey',
  'input:setSpellCheck',
  'kanban:changed',
  'kanban:getConfig',
  'kanban:setConfig',
  'knowledge:applyProposal',
  'knowledge:approveProposal',
  'knowledge:capabilities',
  'knowledge:changed',
  'knowledge:detectEngine',
  'knowledge:engineStart',
  'knowledge:engineStatus',
  'knowledge:envelopeGet',
  'knowledge:envelopeList',
  'knowledge:envelopeUpsert',
  'knowledge:get',
  'knowledge:getBacklinks',
  'knowledge:getContext',
  'knowledge:getExportPayload',
  'knowledge:getProposal',
  'knowledge:listConnections',
  'knowledge:listLinks',
  'knowledge:listProposals',
  'knowledge:metricsGet',
  'knowledge:migrateNotes',
  'knowledge:proposeMutation',
  'knowledge:publishApply',
  'knowledge:publishDistill',
  'knowledge:publishFinalize',
  'knowledge:publishGetDraft',
  'knowledge:publishList',
  'knowledge:publishPrepare',
  'knowledge:publishUpdateDraft',
  'knowledge:rejectProposal',
  'knowledge:rollbackProposal',
  'knowledge:search',
  'knowledge:snapshotCreate',
  'knowledge:snapshotGet',
  'knowledge:unwatch',
  'knowledge:viewRun',
  'knowledge:viewSetAttribute',
  'knowledge:viewsList',
  'knowledge:watch',
  'labels:changed',
  'labels:create',
  'labels:delete',
  'labels:list',
  'labels:update',
  'logo:getUrl',
  'marketplace:CHANGED',
  'marketplace:catalog',
  'marketplace:install',
  'marketplace:progress',
  'marketplace:refresh',
  'marketplace:remove',
  'marketplace:stats',
  'marketplace:update',
  'memory:addLesson',
  'memory:changed',
  'memory:deleteLesson',
  'memory:export',
  'memory:getContext',
  'memory:getProjectMemory',
  'memory:import',
  'memory:insights',
  'memory:listHistory',
  'memory:listLessons',
  'memory:markOnboarded',
  'memory:promoteLesson',
  'memory:promotionCandidates',
  'memory:updateContext',
  'memory:updateLesson',
  'menu:copy',
  'menu:cut',
  'menu:keyboardShortcuts',
  'menu:maximize',
  'menu:minimize',
  'menu:newChat',
  'menu:newWindow',
  'menu:openSettings',
  'menu:paste',
  'menu:quit',
  'menu:redo',
  'menu:selectAll',
  'menu:toggleDevTools',
  'menu:toggleFocusMode',
  'menu:toggleSidebar',
  'menu:undo',
  'menu:zoomIn',
  'menu:zoomOut',
  'menu:zoomReset',
  'messaging:access:allowPending',
  'messaging:access:dismissPending',
  'messaging:access:getMode',
  'messaging:access:getOwners',
  'messaging:access:getPending',
  'messaging:access:setBindingAccess',
  'messaging:access:setMode',
  'messaging:access:setOwners',
  'messaging:bindingChanged',
  'messaging:disconnect',
  'messaging:forget',
  'messaging:generateCode',
  'messaging:generateSupergroupCode',
  'messaging:getBindings',
  'messaging:getConfig',
  'messaging:getSupergroup',
  'messaging:pendingChanged',
  'messaging:platformStatus',
  'messaging:saveDiscord',
  'messaging:saveLark',
  'messaging:saveTelegram',
  'messaging:testDiscord',
  'messaging:testLark',
  'messaging:testTelegram',
  'messaging:unbind',
  'messaging:unbindBinding',
  'messaging:unbindSupergroup',
  'messaging:updateConfig',
  'messaging:wa:buttonPress',
  'messaging:wa:connect',
  'messaging:wa:disconnect',
  'messaging:wa:incoming',
  'messaging:wa:qr',
  'messaging:wa:register',
  'messaging:wa:send',
  'messaging:wa:sendButtons',
  'messaging:wa:sendFile',
  'messaging:wa:sendTyping',
  'messaging:wa:startConnect',
  'messaging:wa:status',
  'messaging:wa:submitPhone',
  'messaging:wa:uiEvent',
  'messaging:wc:cancelConnect',
  'messaging:wc:startConnect',
  'messaging:wc:uiEvent',
  'messaging:wechat:startConnect',
  'messaging:wechat:submitCode',
  'messaging:wechat:uiEvent',
  'mindmap:enrich',
  'mindmap:pinClear',
  'mindmap:pinLoad',
  'mindmap:pinSave',
  'notes:changed',
  'notes:create',
  'notes:delete',
  'notes:deleteAsset',
  'notes:deleteFolder',
  'notes:getBacklinks',
  'notes:getDailyNote',
  'notes:getRenameImpact',
  'notes:importAsset',
  'notes:list',
  'notes:listAssets',
  'notes:read',
  'notes:rename',
  'notes:renameAsset',
  'notes:renameFolder',
  'notes:save',
  'notes:search',
  'notes:unwatch',
  'notes:updateProperties',
  'notes:watch',
  'notification:getEnabled',
  'notification:navigate',
  'notification:setEnabled',
  'notification:show',
  'oauth:cancel',
  'oauth:complete',
  'oauth:revoke',
  'oauth:start',
  'onboarding:clearClaudeOAuthState',
  'onboarding:clearRoxCloud',
  'onboarding:deferSetup',
  'onboarding:exchangeClaudeCode',
  'onboarding:getAuthState',
  'onboarding:getRoxCloudState',
  'onboarding:hasClaudeOAuthState',
  'onboarding:startClaudeOAuth',
  'onboarding:startMcpOAuth',
  'onboarding:startRoxConnect',
  'onboarding:validateMcp',
  'openclawRuntime:getStatus',
  'openclawRuntime:install',
  'openclawRuntime:provision',
  'openclawRuntime:start',
  'openclawRuntime:stop',
  'orgs:accept',
  'orgs:create',
  'orgs:getIdentity',
  'orgs:invite',
  'orgs:list',
  'orgs:listMembers',
  'orgs:setWorkspaceOrg',
  'orgs:updateIdentity',
  'permissions:defaultsChanged',
  'permissions:getDefaults',
  'pi:getApiKeyProviders',
  'pi:getProviderBaseUrl',
  'pi:getProviderModels',
  'pluginBridge:getProjections',
  'pluginBridge:installBazaar',
  'pluginBridge:listPlugins',
  'pluginBridge:openCompat',
  'pluginBridge:setEnabled',
  'pluginBridge:uninstallBazaar',
  'power:getKeepAwake',
  'power:setKeepAwake',
  'preferences:read',
  'preferences:write',
  'projects:changed',
  'projects:create',
  'projects:delete',
  'projects:deleteAsset',
  'projects:get',
  'projects:getOne',
  'projects:listAssets',
  'projects:update',
  'projects:uploadAsset',
  'releaseNotes:get',
  'releaseNotes:getLatestVersion',
  'remote:testConnection',
  'resources:export',
  'resources:import',
  'rtk:getEnabled',
  'rtk:getGain',
  'rtk:getStatus',
  'rtk:setEnabled',
  'securityAudit:acceptRisk',
  'securityAudit:getLatest',
  'securityAudit:revokeRiskAcceptance',
  'securityAudit:run',
  'server:createWorkspace',
  'server:getActiveSessions',
  'server:getHealth',
  'server:getStatus',
  'server:getWorkspaces',
  'server:homeDir',
  'server:shuttingDown',
  'server:statusChanged',
  'session:event',
  'session:getModel',
  'session:setModel',
  'sessions:bulkChanged',
  'sessions:bulkUpdate',
  'sessions:cancel',
  'sessions:command',
  'sessions:create',
  'sessions:delete',
  'sessions:export',
  'sessions:exportRemoteTransfer',
  'sessions:filesChanged',
  'sessions:get',
  'sessions:getFiles',
  'sessions:getMessages',
  'sessions:getNotes',
  'sessions:getPendingPlanExecution',
  'sessions:getPermissionModeState',
  'sessions:getProvenance',
  'sessions:getUnreadSummary',
  'sessions:import',
  'sessions:importRemoteTransfer',
  'sessions:killShell',
  'sessions:markAllRead',
  'sessions:respondToCredential',
  'sessions:respondToPermission',
  'sessions:searchContent',
  'sessions:sendMessage',
  'sessions:setMemoryMode',
  'sessions:setNotes',
  'sessions:unreadSummaryChanged',
  'sessions:unwatchFiles',
  'sessions:watchFiles',
  'settings:getDefaultThinkingLevel',
  'settings:getEnvOverrides',
  'settings:getNetworkProxy',
  'settings:getServerConfig',
  'settings:getServerStatus',
  'settings:setDefaultThinkingLevel',
  'settings:setEnvOverrides',
  'settings:setNetworkProxy',
  'settings:setServerConfig',
  'settings:setupLlmConnection',
  'settings:testLlmConnectionSetup',
  'shell:openFile',
  'shell:openUrl',
  'shell:showInFolder',
  'siyuan:createEmbedded',
  'siyuan:destroy',
  'siyuan:evaluate',
  'siyuan:focus',
  'siyuan:list',
  'siyuan:removed',
  'siyuan:stateChanged',
  'siyuan:syncBounds',
  'skills:changed',
  'skills:delete',
  'skills:exportToProject',
  'skills:get',
  'skills:getFiles',
  'skills:getUsage',
  'skills:importOmp',
  'skills:openEditor',
  'skills:openFinder',
  'skills:pruneUnused',
  'skills:update',
  'skillsPending:approve',
  'skillsPending:changed',
  'skillsPending:diff',
  'skillsPending:dismiss',
  'skillsPending:list',
  'sources:changed',
  'sources:create',
  'sources:delete',
  'sources:get',
  'sources:getMcpTools',
  'sources:getPermissions',
  'sources:reindex',
  'sources:saveCredentials',
  'sources:search',
  'sources:startOAuth',
  'sources:update',
  'statuses:changed',
  'statuses:list',
  'statuses:reorder',
  'system:homeDir',
  'system:isDebugMode',
  'system:versions',
  'tasks:create',
  'tasks:generate',
  'tasks:generated',
  'tasks:get',
  'tasks:getOutput',
  'tasks:getResults',
  'tasks:list',
  'tasks:pause',
  'tasks:resume',
  'tasks:run',
  'tasks:stop',
  'tasks:validate',
  'theme:appChanged',
  'theme:broadcastPreferences',
  'theme:broadcastWorkspaceTheme',
  'theme:getAllWorkspaceThemes',
  'theme:getApp',
  'theme:getColorTheme',
  'theme:getPresets',
  'theme:getSystemPreference',
  'theme:getWorkspaceColorTheme',
  'theme:loadPreset',
  'theme:preferencesChanged',
  'theme:setColorTheme',
  'theme:setWorkspaceColorTheme',
  'theme:systemChanged',
  'theme:workspaceThemeChanged',
  'toolIcons:getMappings',
  'toolchain:getDisabled',
  'toolchain:setDisabled',
  'toolchain:status',
  'toolchain:statusChanged',
  'toolchain:update',
  'tools:getBrowserToolEnabled',
  'tools:setBrowserToolEnabled',
  'transfer:abort',
  'transfer:chunk',
  'transfer:commit',
  'transfer:start',
  'update:available',
  'update:check',
  'update:dismiss',
  'update:downloadProgress',
  'update:getDismissed',
  'update:getInfo',
  'update:install',
  'views:list',
  'views:save',
  'window:cancelClose',
  'window:close',
  'window:closeRequested',
  'window:confirmClose',
  'window:focusState',
  'window:getFocusState',
  'window:getMode',
  'window:getWorkspace',
  'window:openSessionInNewWindow',
  'window:openWorkspace',
  'window:setTrafficLights',
  'window:switchWorkspace',
  'workgraph:convertConnection',
  'workgraph:createConnection',
  'workgraph:getConnection',
  'workgraph:getHealth',
  'workgraph:getVersion',
  'workgraph:importAdc',
  'workgraph:importAwsProfile',
  'workgraph:importDockerHelper',
  'workgraph:importGitHelper',
  'workgraph:importGithubEnv',
  'workgraph:importKeychain',
  'workgraph:importSshAgent',
  'workgraph:listConnectionAudit',
  'workgraph:listConnectionBindings',
  'workgraph:listConnections',
  'workgraph:previewAdc',
  'workgraph:previewAwsProfiles',
  'workgraph:previewDockerHelper',
  'workgraph:previewGitHelper',
  'workgraph:previewGithubEnv',
  'workgraph:previewKeychain',
  'workgraph:previewSshAgent',
  'workgraph:repairConnection',
  'workgraph:revokeConnection',
  'workgraph:revokeConnectionBinding',
  'workgraph:rotateConnection',
  'workgraph:testConnection',
  'workspace:getPermissions',
  'workspace:readImage',
  'workspace:writeImage',
  'workspaceSettings:get',
  'workspaceSettings:update',
  'workspaces:checkSlug',
  'workspaces:create',
  'workspaces:get',
  'workspaces:updateRemote',
]

const EXPECTED_COUNT = EXPECTED_CHANNELS.length
// End auto-generated

describe('RPC_CHANNELS wire-format stability', () => {
  it(`contains exactly ${EXPECTED_COUNT} channel strings`, () => {
    const actual = flattenValues(RPC_CHANNELS)
    expect(actual.length).toBe(EXPECTED_COUNT)
  })

  it('preserves exact wire-format strings (sorted equality)', () => {
    const actual = flattenValues(RPC_CHANNELS).sort()
    expect(actual).toEqual(EXPECTED_CHANNELS)
  })

  it('has no duplicate wire-format strings', () => {
    const values = flattenValues(RPC_CHANNELS)
    const unique = new Set(values)
    expect(values.length).toBe(unique.size)
  })
})

// ── BroadcastEventMap payload shape assertions ──────────────────────────
// These compile-time checks prevent the emitter/listener payload mismatch
// that caused sources:changed to silently send undefined after OAuth flows.
type AssertTuple<T extends readonly unknown[], N extends number> =
  T['length'] extends N ? true : false

describe('BroadcastEventMap payload shapes', () => {
  it('sources:changed carries (workspaceId, sources)', () => {
    type Payload = BroadcastEventMap[typeof RPC_CHANNELS.sources.CHANGED]
    const _check: AssertTuple<Payload, 2> = true
    expect(_check).toBe(true)
  })

  it('skills:changed carries (workspaceId, skills)', () => {
    type Payload = BroadcastEventMap[typeof RPC_CHANNELS.skills.CHANGED]
    const _check: AssertTuple<Payload, 2> = true
    expect(_check).toBe(true)
  })

  it('contextDocs:CHANGED carries no payload', () => {
    type Payload = BroadcastEventMap[typeof RPC_CHANNELS.contextDocs.CHANGED]
    const _check: AssertTuple<Payload, 0> = true
    expect(_check).toBe(true)
  })
})
