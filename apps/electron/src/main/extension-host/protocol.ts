...
export const SECRET_ENV_KEY_RE =
  /^(?:.*(?:API[_-]?KEY|SECRET|TOKEN|PASSWORD|PASSWD|CREDENTIAL|PRIVATE[_-]?KEY).*|.*_KEY$|AWS_SECRET_ACCESS_KEY|AWS_ACCESS_KEY_ID|AWS_SESSION_TOKEN|OPENAI_API_KEY|ANTHROPIC_API_KEY|GOOGLE_API_KEY|GOOGLE_OAUTH_CLIENT_SECRET|SLACK_OAUTH_CLIENT_SECRET|GITHUB_TOKEN|GH_TOKEN|NPM_TOKEN|HF_TOKEN|HUGGING_FACE_HUB_TOKEN)$/i
